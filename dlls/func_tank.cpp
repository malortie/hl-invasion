...
void CFuncTank :: Think( void )
{
	...
	if ( std::abs(pev->avelocity.x) > 1 || std::abs(pev->avelocity.y) > 1 )
		StartRotSound();
	else
		StopRotSound();
}
...
//		direction = m_sightOrigin - barrelEnd;
	...
	if ( CanFire() && ( (std::abs(distX) < m_pitchTolerance && std::abs(distY) < m_yawTolerance) || (pev->spawnflags & SF_TANK_LINEOFSIGHT) ) )
	{
		BOOL fire = FALSE;
		Vector forward;
...
void CFuncTank::AdjustAnglesForBarrel( Vector &angles, float distance )
{
	...
	if ( m_barrelPos.y != 0 || m_barrelPos.z != 0 )
	{
		...
		if ( m_barrelPos.y )
		{
			...
			angles.y += (180.0 / M_PI) * std::atan2( m_barrelPos.y, std::sqrt( d2 - r2 ) );
		}
		if ( m_barrelPos.z )
		{
			r2 = m_barrelPos.z * m_barrelPos.z;
			angles.x += (180.0 / M_PI) * std::atan2( -m_barrelPos.z, std::sqrt( d2 - r2 ) );
		}
	}
}
