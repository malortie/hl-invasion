...
void CFuncVgui :: KeyValue( KeyValueData *pkvd )
{
	...
	{
		m_iparam1 = std::atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "iparam2") )
	{
		m_iparam2 = std::atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else if (FStrEq(pkvd->szKeyName, "iparam3") )
	{
		m_iparam3 = std::atoi( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else
	{
