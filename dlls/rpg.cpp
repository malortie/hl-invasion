	...
	if ( m_pTargetMonster != NULL )
	{
		...
		if (tr.flFraction >= 0.90)
		{
			...
			else
			{
				...
				if ( std::abs(flAngDistX) < 45 && std::abs(flAngDistY) < 45 )
				{

					if ( std::abs(flAngDistX) <= 9 && std::abs(flAngDistY) <= 9 )
					{
						vecTarget = vecDir;
					}
					else
...
void CRpg ::  ItemTouch( CBaseEntity *pOther )
{
	...
	for ( int i = 0 ; i < MAX_ITEM_TYPES ; i++ )
	{
		...
		while (pItem)
		{
			if ( !std::strcmp( "weapon_rpg", STRING( pItem->pev->classname ) ) )
			{
...
class CRpgAmmo : public CBasePlayerAmmo
{
	...
	{ 
		...
		for ( i = 0 ; i < MAX_ITEM_TYPES ; i++ )
		{
			...
			while (pItem)
			{
				if ( !std::strcmp( "weapon_rpg"/*&pszItemName*/, STRING( pItem->pev->classname ) ) )
				{
					fin = 1;
					pRpg = (CRpg*)pItem->GetWeaponPtr();
				}
