...
//			ALERT ( at_console, "MELEE1\n" );
			...
			m_iFear = std::max ( HATE_LEVEL, m_iFear - 5 );
...
//			ALERT ( at_console, "ATTACK1\n" );
			...
			m_iFear = std::min ( FEAR_LEVEL, m_iFear + 5 );
...
//			ALERT ( at_console, "ATTACK2\n" );
		...
		{
			m_iFear = std::min ( FEAR_LEVEL, m_iFear + 3 );

			m_flightSpeed = std::min ( m_flMaxSpeed, m_flightSpeed + 40 );
		}
...
Schedule_t* CFlybee :: GetScheduleOfType ( int Type ) 
{
	...
	case SCHED_RANGE_ATTACK2:
		...
		{
			if ( (Center()-m_hEnemy->Center()).Length() <= 300 && std::abs(pev->origin.z-m_hEnemy->pev->origin.z) <= 128 )
...
//				ALERT ( at_console, "RUNATTACK fails\n" );
			...
			m_flightSpeed = std::min ( m_flMaxSpeed, m_flightSpeed *= 1.2 );
