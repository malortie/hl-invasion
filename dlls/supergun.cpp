...
void CSGBall :: AnimateThink( void  )
{
	...
	if ( m_hEnemy != NULL && FVisible( m_hEnemy ))
	{
		...
		if ( std::abs ( difX ) < 15 && std::abs ( difY ) < 15 )
		{
...
void CSuperGun::SecondaryAttack()
{
	...
	int iProjectiles = std::min ( m_iClip, 8 );	// n'en tire pas plus qu'il n'en a ...
	...
	m_pPlayer->m_flNextAttack = UTIL_WeaponTimeBase() + 31 / 20.0;
