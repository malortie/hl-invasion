...
*/
	...
	while ( p != NULL )
	{
		...
		float flDifX = (float)std::abs( (int)(vecAngFlash.x-vecAngDelta.x)%360 );
		flDifX = flDifX > 180 ? flDifX - 360 : flDifX;
		flDifX = flDifX < -180 ? flDifX + 360 : flDifX;

		float flDifY = (float)std::abs( (int)(vecAngDelta.y-vecAngFlash.y)%360 );
		flDifY = flDifY > 180 ? flDifY - 360 : flDifY;
		flDifY = flDifY < -180 ? flDifY + 360 : flDifY;

		if ( std::abs( flDifX ) > 45 ||	std::abs( flDifY ) > 45 )
		{
			p = p->pNext;
			continue;
		}
		...
		float pixelDist = std::sqrt ( (vecScrPos[0]-XRES(320))*(vecScrPos[0]-XRES(320))
								+(vecScrPos[1]-YRES(240))*(vecScrPos[1]-YRES(240)) );

		float maxdist = std::sqrt ( XRES(330)*XRES(330) + YRES(250)*YRES(250) );
		...
		brightnessratio *= std::abs( flDifY ) < 25 ? 1 : 1 - ( (std::abs(flDifY)-25)/20 );
		brightnessratio *= std::abs( flDifX ) < 25 ? 1 : 1 - ( (std::abs(flDifX)-25)/20 );
